...
const MyRatings = () => {
    ...
    useEffect(() => {
        ...
        const fetchRatings = async () => {
            try {
                ...
                setError('Gagal memuat rating Anda.');
            } finally {
                setLoading(false);
            }
        };
    ...
    if (!isLoggedIn) return <div style={{ textAlign: 'center', marginTop: '2rem' }}>Silakan login untuk melihat rating Anda.</div>;
                        ...
                        <div key={rating.id} style={{ display: 'flex', alignItems: 'center', border: '1px solid #eee', padding: '1rem', borderRadius: '8px', background: '#f9f9f9' }}>
                            <Link to={`/recipes/${rating.recipe.id}`} style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', color: 'inherit', flexGrow: 1 }}>
                                ...
                                <div>
                                    ...
                                    {/* Pass onRatingChange as a dummy or re-evaluate average on change if needed */}
